...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', -apple-system, BlinkMacSystemFont, Roboto, sans-serif; }
        ...
        .modal-content { background-color: var(--card-bg); color: var(--text-color); padding: 24px; border-radius: 14px; width: 440px; max-width: 90%; box-shadow: 0 20px 40px rgba(0,0,0,0.25); border: 1px solid var(--border-color); }
        ...
        .form-group input, .form-group select, .form-group textarea { width: 100%; padding: 9px; border: 1px solid var(--border-color); border-radius: 6px; font-size: 13px; background: var(--card-bg); color: var(--text-color); outline: none; }
    ...
    <div id="modalTambahProyek" class="modal">
        <div class="modal-content" style="width: 580px; max-width: 95%;">
            <div class="modal-header">
                <h3><i class="fas fa-project-diagram" style="color: #0284c7;"></i> Buat Proyek Baru SMTI</h3>
                <span class="close-modal" onclick="closeAddProjectModal()">&times;</span>
            </div>
            <form id="form-tambah-proyek" onsubmit="saveNewProject(event)">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="new-proj-pic" required placeholder="Contoh: Dion / Tim SMTI">
                    </div>
                </div>
                ...
                <div class="form-group">
                    <label><i class="fas fa-sitemap"></i> Template Alur Kerja</label>
                    <select id="new-proj-template" onchange="onTemplateChange(this.value)">
                        <option value="MIKU">Template MIKU (5 Tahap: Sosialisasi, Seleksi Ide, Prototyping, Coaching, Final Pitching)</option>
                        <option value="PMSMT">Template PMSMT (5 Tahap: Gap Analysis, Pra-Audit, Audit Eksternal, Tindak Lanjut CAR, Sertifikasi)</option>
                        <option value="PSP">Template Pengembangan Sistem & Prosedur (5 Tahap: Identifikasi, Drafting, Uji Coba, Pengesahan, Distribusi)</option>
                        <option value="manual">Manual / Kustom Sendiri (Input & Tentukan Tahap Bebas)</option>
                    </select>
                </div>

                <!-- Interactive Custom Steps Builder -->
                <div class="form-group" style="background: var(--hover-color); padding: 14px; border-radius: 8px; border: 1px solid var(--border-color);">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                        <label style="margin: 0; font-weight: 700; font-size: 12.5px;">
                            <i class="fas fa-list-ol"></i> Rincian Tahapan Alur Kerja (Dapat Diedit Manual):
                        </label>
                    ...
                    <div id="custom-steps-list" style="display: flex; flex-direction: column; gap: 8px; max-height: 200px; overflow-y: auto; padding-right: 4px;">
                        <!-- Dynamic Step Rows -->
                    </div>
                    <small style="color: #64748b; font-size: 11px; display: block; margin-top: 8px;">
                        * Pilih template di atas atau ketik nama tahap manual. Anda dapat menambah/menghapus tahap semau Anda.
                    </small>
                </div>

                <div style="display: flex; gap: 10px; justify-content: flex-end; margin-top: 18px; border-top: 1px solid var(--border-color); padding-top: 14px;">
                    <button type="button" class="btn" style="background: #64748b;" onclick="closeAddProjectModal()">Batal</button>
                    <button type="submit" class="btn" style="background: #0284c7;"><i class="fas fa-save"></i> Buat & Daftarkan Proyek</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function renderProjectTeam(project) {
            ...
            container.innerHTML = project.teamMembers.map(name => {
                const emp = SMTI_EMPLOYEES.find(e => e.name.toLowerCase() === name.toLowerCase()) || {
                    name: name,
                    role: "Anggota Tim",
                ...
                return `
                    <div class="team-member-pill" title="${emp.name} (${emp.role})">
                        <div class="team-avatar-circle" style="background: ${emp.color};">
                            ${emp.initials}
                        </div>
                        <span>${emp.name}</span>
                        <button type="button" class="team-member-remove" onclick="removeMemberFromProject('${emp.name.replace(/'/g, "\\'")}')" title="Keluarkan dari tim proyek">
        ...
        function openInviteTeamModal() {
            ...
            list.innerHTML = SMTI_EMPLOYEES.map(emp => {
                const isInvited = project.teamMembers.some(m => m.toLowerCase() === emp.name.toLowerCase());
                return `
                    <div style="display: flex; justify-content: space-between; align-items: center; background: var(--hover-color); padding: 10px 14px; border-radius: 8px; border: 1px solid var(--border-color);">
                        <div style="display: flex; align-items: center; gap: 10px;">
        ...
        function populateChatAuthorSelect(project) {
            ...
            if (invited.length > 0) {
                ...
                invited.forEach(mName => {
                    const emp = SMTI_EMPLOYEES.find(e => e.name.toLowerCase() === mName.toLowerCase()) || { name: mName, role: "Anggota Tim" };
                    optionsHtml += `<option value="${emp.name}">${emp.name} (${emp.role})</option>`;
                });
                optionsHtml += `</optgroup>`;
            }

            const others = SMTI_EMPLOYEES.filter(e => !invited.some(m => m.toLowerCase() === e.name.toLowerCase()));
            if (others.length > 0) {
                optionsHtml += `<optgroup label="Karyawan SMTI Lainnya">`;
                others.forEach(emp => {
                    optionsHtml += `<option value="${emp.name}">${emp.name} (${emp.role})</option>`;
                });
                optionsHtml += `</optgroup>`;
        ...
        function populateNewProjectMemberCheckboxes() {
            ...
            container.innerHTML = SMTI_EMPLOYEES.map(emp => `
        ...
        /* =========================================================
           FITUR TAMBAH PROYEK (3 KATEGORI & BUILDER ALUR MANUAL)
           ========================================================= */
        ...
        const templatePresets = {
            MIKU: [
                { title: "Sosialisasi & Penjaringan Ide", date: "Tahap 1", desc: "Penyebaran pedoman dan pengumpulan ide awal dari peserta." },
                { title: "Seleksi & Verifikasi Ide", date: "Tahap 2", desc: "Verifikasi orisinalitas ide dan kelayakan bisnis/teknis." },
                { title: "Pembuatan Prototype & Mockup", date: "Tahap 3", desc: "Pendampingan penyusunan bentuk prototipe fisik / sistem." },
                { title: "Coaching Presentasi Pitch", date: "Tahap 4", desc: "Latihan dan gladi resik pitching sebelum dewan juri." },
                { title: "Final Pitching & Awarding", date: "Tahap 5", desc: "Presentasi akhir dan pengumuman penetapan pemenang." }
            ],
            PMSMT: [
                { title: "Gap Analysis & Dokumen Standar", date: "Tahap 1", desc: "Pemetaan kesenjangan klausul standar dan pembaruan prosedur/IK." },
                { title: "Pra-Audit & Penyisiran Internal", date: "Tahap 2", desc: "Pemeriksaan kesiapan di seluruh unit kerja terkait." },
                { title: "Audit Eksternal Lembaga Akreditasi", date: "Tahap 3", desc: "Audit resmi oleh lembaga sertifikasi independen." },
                { title: "Tindak Lanjut Temuan (CAR)", date: "Tahap 4", desc: "Penyelesaian dan verifikasi tindakan perbaikan." },
                { title: "Penerbitan Sertifikat & Evaluasi", date: "Tahap 5", desc: "Penerbitan sertifikat akreditasi resmi KAN." }
            ],
            PSP: [
                { title: "Identifikasi Kebutuhan Prosedur", date: "Tahap 1", desc: "Pemetaan proses bisnis dan inventarisasi SOP/IK yang dibutuhkan." },
                { title: "Drafting & Konsultasi Stakeholder", date: "Tahap 2", desc: "Penyusunan draft prosedur bersama unit kerja terkait." },
                { title: "Simulasi & Uji Coba Prosedur", date: "Tahap 3", desc: "Uji coba penerapan prosedur di lapangan/kantor." },
                { title: "Review & Pengesahan Manajemen", date: "Tahap 4", desc: "Pemeriksaan format dan penandatanganan resmi pimpinan." },
                { title: "Penerbitan & Sosialisasi Resmi", date: "Tahap 5", desc: "Pemberian kode dokumen resmi dan upload portal SMTI." }
            ],
            manual: [
                { title: "Tahap 1: Inisiasi & Perencanaan", date: "Bulan 1", desc: "Perencanaan dan kick-off kegiatan." },
                { title: "Tahap 2: Eksekusi & Implementasi", date: "Bulan 2", desc: "Pelaksanaan tugas utama." },
                { title: "Tahap 3: Evaluasi & Pelaporan Akhir", date: "Bulan 3", desc: "Review hasil dan serah terima." }
            ]
        };

        function onTemplateChange(tplKey) {
            const preset = templatePresets[tplKey] || templatePresets.manual;
            customStepsState = JSON.parse(JSON.stringify(preset));
        ...
        function renderCustomStepsRows() {
            ...
            container.innerHTML = customStepsState.map((step, idx) => `
                <div style="display: flex; gap: 8px; align-items: center; background: var(--card-bg); padding: 7px 10px; border-radius: 6px; border: 1px solid var(--border-color);">
                    <span style="font-size: 11px; font-weight: 700; width: 55px; color: var(--secondary-color); white-space: nowrap;">Tahap ${idx + 1}</span>
                    <input type="text" value="${step.title.replace(/"/g, '&quot;')}" oninput="customStepsState[${idx}].title = this.value" placeholder="Nama Tahap..." style="flex: 2; padding: 6px 10px; border: 1px solid var(--border-color); border-radius: 4px; font-size: 12px; background: var(--bg-color); color: var(--text-color); outline: none;">
                    <input type="text" value="${step.date.replace(/"/g, '&quot;')}" oninput="customStepsState[${idx}].date = this.value" placeholder="Target waktu..." style="flex: 1; padding: 6px 10px; border: 1px solid var(--border-color); border-radius: 4px; font-size: 12px; background: var(--bg-color); color: var(--text-color); outline: none;">
                    <button type="button" onclick="removeCustomStepRow(${idx})" style="background: transparent; border: none; color: #ef4444; cursor: pointer; padding: 4px;" title="Hapus tahap ini">
                        <i class="fas fa-trash-alt"></i>
                    </button>
                </div>
            `).join('');
        ...
        function addCustomStepRow() {
            customStepsState.push({
                title: `Tahap ${customStepsState.length + 1}: Kegiatan Baru`,
        ...
        function saveNewProject(e) {
            ...
            // Construct flow from the user's customized steps
            const flow = customStepsState.map((st, i) => {
                return {
                    ...
                    tasks: [
                        [`Persiapan dan verifikasi dokumen tahap ${i + 1}`, false]
                    ]
                };
            });
            ...
            if (checkedMembers.length === 0 && pic) {
                checkedMembers.push(pic.split('/')[0].trim());
            }
