...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', -apple-system, BlinkMacSystemFont, Roboto, sans-serif; -webkit-tap-highlight-color: transparent; }
        ...
        .cloud-sync-badge {
            display: none !important; /* Disembunyikan sesuai permintaan */
            align-items: center;
            gap: 6px;
            padding: 6px 12px;
            border-radius: 20px;
        ...
        #notif-dropdown { width: 340px; max-width: 92vw; padding: 12px; }
        ...
        .card[onclick]:hover { transform: translateY(-3px); box-shadow: 0 6px 14px rgba(0,0,0,0.08); }
        ...
        .mon-pill-btn.mon-pill-completed.active {
            background: #10b981;
            color: #ffffff;
            border-color: #34d399;
        ...
        .mon-pill-btn.mon-pill-completed:hover:not(.active) {
        ...
        .monitor-col-completed { border-top: 4px solid #10b981; background: rgba(16, 185, 129, 0.04); }
        ...
        .monitor-col-completed .monitor-col-header h3 { color: #34d399; }
        ...
        /* =========================================================
           EXECUTIVE ANALYTICS MULTI-CHART & INFOGRAPHIC STYLES
           (Inspired by flat-infographic-element-set palette & shapes)
           ========================================================= */
        .btn-infographic-pill {
            display: inline-flex;
            align-items: center;
            gap: 12px;
        ...
        .btn-infographic-pill:hover {
            transform: translateY(-2px);
            opacity: 0.96;
        }
        .btn-infographic-pill:active {
            transform: translateY(0);
        }
        .btn-infographic-pill .pill-icon-bubble {
            width: 32px;
            height: 32px;
            border-radius: 50%;
            background: #ffffff;
        ...
        .btn-infographic-pill:hover .pill-icon-bubble {
            transform: scale(1.08);
        }
        .btn-infographic-sunset {
            background: linear-gradient(90deg, #ec4899 0%, #8b5cf6 100%);
            box-shadow: 0 4px 16px rgba(236, 72, 153, 0.35);
        }
        .btn-infographic-sunset .pill-icon-bubble {
            color: #ec4899;
        }
        .btn-infographic-ocean {
            background: linear-gradient(90deg, #0284c7 0%, #10b981 100%);
            box-shadow: 0 4px 16px rgba(2, 132, 199, 0.35);
        }
        .btn-infographic-ocean .pill-icon-bubble {
            color: #0284c7;
        }

        /* Infographic Modern KPI Card */
        .infographic-kpi-card {
            transition: transform 0.22s ease, box-shadow 0.22s ease;
        }
        .infographic-kpi-card:hover {
            transform: translateY(-3px);
            box-shadow: 0 10px 25px rgba(0, 0, 0, 0.08);
        }
        body.dark-mode .infographic-kpi-card {
            background: linear-gradient(145deg, #1e1e38 0%, #151528 100%) !important;
        ...
        body.dark-mode .analytics-chart-box {
            background: linear-gradient(145deg, #18182f 0%, #121224 100%);
            border: 1px solid rgba(255, 255, 255, 0.08);
...
<body>
    ...
    <div class="main-content">
        <div class="header">
            ...
            <!-- Notif Dropdown (Disinkronkan dinamis dengan proyek berjalan) -->
            <div id="notif-dropdown" class="dropdown-menu">
                <!-- Rendered dynamically by renderNotifications() -->
            </div>
        ...
        <div id="dasbor" class="page-section active">
            <div class="dashboard-grid">
                <div class="card" style="border-left: 4px solid #0284c7; cursor: pointer; transition: transform 0.2s;" onclick="showPage('proyek', document.querySelectorAll('.menu-item')[1])" title="Klik untuk membuka Daftar Proyek Berjalan">
                    <div class="card-header"><span>Total Proyek SMTI</span><i class="fas fa-tasks" style="color: #0284c7;"></i></div>
                    <div style="display: flex; align-items: baseline; gap: 8px;">
                        <h2 id="dash-total-proyek" style="font-size: 30px; font-weight: 800;">11</h2>
                        <span id="dash-running-badge" style="font-size: 12.5px; font-weight: 700; color: #0284c7; background: rgba(2,132,199,0.12); padding: 2px 7px; border-radius: 6px;">8 Berjalan</span>
                    </div>
                    <small id="dash-proyek-subtext" style="color: #64748b;">8 Sedang Berjalan • 3 Selesai</small>
                </div>
                <div class="card" style="border-left: 4px solid #dc2626; cursor: pointer; transition: transform 0.2s;" onclick="showPage('proyek', document.querySelectorAll('.menu-item')[1]); filterProjects('Tinggi', document.querySelector('.tab-btn[data-filter=\'Tinggi\']'))" title="Klik untuk memfilter proyek urgensi tinggi">
                    <div class="card-header"><span>Urgensi Tinggi (Aktif)</span><i class="fas fa-fire" style="color: #dc2626;"></i></div>
                    <h2 id="dash-urgent-proyek" style="font-size: 30px; font-weight: 800; color: #dc2626;">2</h2>
                    <small style="color: #dc2626; font-weight: 600;"><i class="fas fa-clock"></i> Membutuhkan Tindakan Segera</small>
                </div>
                <div class="card" style="border-left: 4px solid #d97706; cursor: pointer; transition: transform 0.2s;" onclick="showPage('proyek', document.querySelectorAll('.menu-item')[1]); filterProjects('Sedang', document.querySelector('.tab-btn[data-filter=\'Sedang\']'))" title="Klik untuk memfilter proyek urgensi sedang">
                    <div class="card-header"><span>Urgensi Sedang (Aktif)</span><i class="fas fa-hourglass-half" style="color: #d97706;"></i></div>
                ...
                <div class="card" style="border-left: 4px solid #16a34a; cursor: pointer; transition: transform 0.2s;" onclick="if(isCurrentUserSuperAdmin()) showPage('karyawan', document.getElementById('menu-item-karyawan'))" title="Klik untuk melihat personil SMTI">
                    <div class="card-header"><span>Personil Dept SMTI</span><i class="fas fa-users" style="color: #16a34a;"></i></div>
                    <h2 id="dash-karyawan-count" style="font-size: 30px; font-weight: 800;">12</h2>
                    <small id="dash-karyawan-subtext" style="color: #64748b;">8 TKO • 4 TKNO</small>
                </div>
            </div>
            ...
            <!-- Live Status Progres Proyek Berjalan Terkini di Dasbor -->
            <div class="card">
                <div class="card-header" style="margin-bottom: 12px;">
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <i class="fas fa-stream" style="color: #0284c7; font-size: 16px;"></i>
                        <span style="font-size: 15px; font-weight: 700;">Status Progres Proyek Berjalan Terkini</span>
                    </div>
                    <button class="btn btn-sm" style="background: #0284c7;" onclick="showPage('proyek', document.querySelectorAll('.menu-item')[1])">
                        Lihat Seluruh Proyek <i class="fas fa-arrow-right"></i>
                    </button>
                </div>
                <div id="dash-running-projects-list" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 12px;">
                    <!-- Rendered dynamically by renderDashboardRunningProjects() -->
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px;">
                    <div>
                        <div style="display: flex; align-items: center; gap: 8px;">
                            <i id="main-chart-icon" class="fas fa-chart-line" style="color: #0284c7; font-size: 16px;"></i>
                            <span id="main-chart-title" style="font-size: 15px; font-weight: 700;">Tren Kinerja & Progres Akumulatif Seluruh Dept SMTI</span>
                        </div>
            ...
            <div class="card" style="margin-bottom: 16px;">
                <div class="card-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
                    ...
                    <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                        <button class="btn-infographic-pill btn-infographic-sunset" onclick="openPrintReportModal()">
                            <span>Unduh Laporan PDF Resmi</span>
                            <div class="pill-icon-bubble"><i class="fas fa-file-pdf"></i></div>
                        </button>
                        <button class="btn-infographic-pill btn-infographic-ocean" onclick="exportAllProjectsCSV()">
                            <span>Export Rekap CSV</span>
                            <div class="pill-icon-bubble"><i class="fas fa-file-excel"></i></div>
                        </button>
                    </div>
                </div>

                <!-- 4 Live Dynamic KPI Cards (Infographic Modern Style) -->
                <div class="dashboard-grid" style="margin-top: 18px; margin-bottom: 0;">
                    <!-- Card 1: Ketuntasan -->
                    <div class="card infographic-kpi-card" style="margin: 0; padding: 18px; border-radius: 14px; position: relative; overflow: hidden; border: 1px solid var(--border-color); background: var(--card-bg);">
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <span style="font-size: 11px; text-transform: uppercase; font-weight: 700; letter-spacing: 0.5px; opacity: 0.75;">Tingkat Ketuntasan</span>
                            <div style="width: 32px; height: 32px; border-radius: 8px; background: rgba(16, 185, 129, 0.12); display: flex; align-items: center; justify-content: center;">
                                <i class="fas fa-check-double" style="color: #10b981; font-size: 15px;"></i>
                            </div>
                        </div>
                        <h2 id="report-kpi-completion" style="font-size: 28px; font-weight: 800; color: var(--text-color); margin-top: 6px;">--%</h2>
                        <small id="report-kpi-completion-sub" style="color: #10b981; font-weight: 700; font-size: 11px;">Memuat data...</small>
                    </div>

                    <!-- Card 2: Rata-rata Progres -->
                    <div class="card infographic-kpi-card" style="margin: 0; padding: 18px; border-radius: 14px; position: relative; overflow: hidden; border: 1px solid var(--border-color); background: var(--card-bg);">
                        <div style="position: absolute; top: 0; left: 0; right: 0; height: 4px; background: linear-gradient(90deg, #0284c7, #6366f1);"></div>
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <span style="font-size: 11px; text-transform: uppercase; font-weight: 700; letter-spacing: 0.5px; opacity: 0.75;">Rata-rata Progres SMTI</span>
                            <div style="width: 32px; height: 32px; border-radius: 8px; background: rgba(2, 132, 199, 0.12); display: flex; align-items: center; justify-content: center;">
                                <i class="fas fa-chart-line" style="color: #0284c7; font-size: 15px;"></i>
                            </div>
                        </div>
                        <h2 id="report-kpi-avg" style="font-size: 28px; font-weight: 800; color: #0284c7; margin-top: 6px;">--%</h2>
                        <small id="report-kpi-avg-sub" style="color: #64748b; font-size: 11px;">Evaluasi Realisasi 2026</small>
                    </div>

                    <!-- Card 3: Proyek Kritis / Berjalan -->
                    <div id="report-kpi-urgent-card" class="card infographic-kpi-card" style="margin: 0; padding: 18px; border-radius: 14px; position: relative; overflow: hidden; border: 1px solid var(--border-color); background: var(--card-bg);">
                        <div id="report-kpi-urgent-bar" style="position: absolute; top: 0; left: 0; right: 0; height: 4px; background: linear-gradient(90deg, #10b981, #14b8a6);"></div>
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <span id="report-kpi-urgent-title" style="font-size: 11px; text-transform: uppercase; font-weight: 700; letter-spacing: 0.5px; color: #10b981;">Proyek Kritis / Berjalan</span>
                            <div id="report-kpi-urgent-icon-wrap" style="width: 32px; height: 32px; border-radius: 8px; background: rgba(16, 185, 129, 0.12); display: flex; align-items: center; justify-content: center;">
                                <span id="report-kpi-urgent-icon"><i class="fas fa-shield-alt" style="color: #10b981; font-size: 15px;"></i></span>
                            </div>
                        </div>
                        <h2 id="report-kpi-urgent" style="font-size: 28px; font-weight: 800; color: #10b981; margin-top: 6px;">0 Proyek</h2>
                        <small id="report-kpi-urgent-sub" style="color: #10b981; font-weight: 700; font-size: 11px;">Semua Terkendali Aman</small>
                    </div>

                    <!-- Card 4: Kontributor Terlibat -->
                    <div class="card infographic-kpi-card" style="margin: 0; padding: 18px; border-radius: 14px; position: relative; overflow: hidden; border: 1px solid var(--border-color); background: var(--card-bg);">
                        <div style="position: absolute; top: 0; left: 0; right: 0; height: 4px; background: linear-gradient(90deg, #ec4899, #8b5cf6);"></div>
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <span style="font-size: 11px; text-transform: uppercase; font-weight: 700; letter-spacing: 0.5px; opacity: 0.75;">Kontributor Terlibat</span>
                            <div style="width: 32px; height: 32px; border-radius: 8px; background: rgba(139, 92, 246, 0.12); display: flex; align-items: center; justify-content: center;">
                                <i class="fas fa-users-cog" style="color: #8b5cf6; font-size: 15px;"></i>
                            </div>
                        </div>
                        <h2 id="report-kpi-team" style="font-size: 28px; font-weight: 800; color: #8b5cf6; margin-top: 6px;">--</h2>
            ...
            <div class="analytics-grid-2x2">
                ...
                <div class="analytics-chart-box">
                    <div class="analytics-chart-header">
                        <span><i class="fas fa-chart-pie" style="color: #ec4899;"></i> Komposisi Status Portofolio Proyek</span>
                        <small>Persentase & Jumlah</small>
                    </div>
                    <div class="analytics-chart-canvas-wrap" style="position: relative;">
                        <canvas id="chartReportStatus"></canvas>
                        <div id="chartReportStatusCenter" style="position: absolute; top: 40%; left: 50%; transform: translate(-50%, -50%); text-align: center; pointer-events: none;">
                            <span style="font-size: 10.5px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; opacity: 0.65; display: block;">Ketuntasan</span>
                            <strong id="chartReportStatusCenterVal" style="font-size: 24px; font-weight: 800; background: linear-gradient(135deg, #10b981, #06b6d4); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">--%</strong>
                            <small id="chartReportStatusCenterSub" style="font-size: 10px; display: block; opacity: 0.75; font-weight: 600;">Memuat...</small>
                        </div>
                    </div>
                </div>
        ...
        <div class="monitor-board">
            ...
            <div class="monitor-column monitor-col-high" id="mon-col-high">
                <div class="monitor-col-header">
                    ...
                    <span class="monitor-col-badge" id="mon-count-high">0 Proyek</span>
                </div>
            ...
            <div class="monitor-column monitor-col-med" id="mon-col-med">
                <div class="monitor-col-header">
                    ...
                    <span class="monitor-col-badge" id="mon-count-med">0 Proyek</span>
                </div>
            ...
            <!-- Kolom 4: Arsip Proyek Sudah Selesai (Tuntas 100%) -->
            <div class="monitor-column monitor-col-completed" id="mon-col-completed" style="display: none;">
                <div class="monitor-col-header">
                    <h3><i class="fas fa-check-double"></i> ARSIP PROYEK SUDAH SELESAI (100% TUNTAS)</h3>
                    <span class="monitor-col-badge" id="mon-count-completed" style="background: rgba(16, 185, 129, 0.2); color: #34d399; border: 1px solid rgba(16, 185, 129, 0.4);">0 Proyek</span>
                </div>
                <div class="monitor-card-list" id="monitor-list-completed">
                    <!-- Dynamic -->
                </div>
            </div>
        </div>
    ...
    <div id="flowModal">
        <div class="flow-modal-box">
            ...
            <div class="flow-modal-body">
                <!-- Banner Notifikasi Hak Akses Proyek (Read-Only / Full Access) -->
                <div id="modal-permission-notice" style="display: none;"></div>

                <!-- Toolbar Cepat Pengatur Status Tahap & Manual Progress (Khusus PIC & Tim) -->
                <div class="stage-control-toolbar" id="modal-stage-toolbar">
                ...
                <div class="flow-meta-grid">
                    ...
                    <div class="flow-meta-item" style="position: relative;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2px;">
                            ...
                            <button type="button" class="btn-xs" id="btn-modal-change-pic" style="background: rgba(2, 132, 199, 0.1); border: 1px solid rgba(2, 132, 199, 0.3); color: #0284c7; padding: 2px 7px; border-radius: 4px; font-size: 10px; cursor: pointer; font-weight: 700;" onclick="openChangePicModal()" title="Ubah Penanggung Jawab Proyek">
                ...
                <div class="project-team-section">
                    ...
                    <button type="button" class="btn btn-sm btn-invite-team" id="btn-modal-invite-team" onclick="openInviteTeamModal()">
                ...
                <div class="active-stage-detail" id="modal-stage-detail-box">
                    ...
                    <div class="stage-alert-box" id="modal-stage-alert" style="display: none;">
                        ...
                        <button class="btn btn-sm" id="btn-modal-edit-alert" style="background: rgba(220, 38, 38, 0.2); color: #dc2626;" onclick="editAlertNote()">
                    ...
                    <div class="task-checklist">
                        ...
                        <!-- Form Input Manual Tambah Checklist Baru (Hanya untuk PIC & Tim) -->
                        <form class="add-task-form" id="modal-add-task-form" onsubmit="event.preventDefault(); addNewTask();">
                        ...
                        <div id="modal-task-locked-notice" style="display: none; padding: 10px 14px; background: rgba(148, 163, 184, 0.08); border: 1px dashed var(--border-color); border-radius: 6px; text-align: center; color: #64748b; font-size: 12px; margin-top: 6px;">
                            <i class="fas fa-lock" style="color: #94a3b8; margin-right: 5px;"></i> Penambahan kegiatan baru disembunyikan (khusus PIC & anggota tim ter-invite).
                        </div>
                    </div>
                </div>
                ...
                <div class="flow-modal-actions">
                    <div style="display: flex; gap: 8px; flex-wrap: wrap;">
                        <button class="btn btn-sm" id="btn-modal-mark-completed" style="background: #16a34a; font-weight: 700;" onclick="markProjectCompletedQuick()" title="Langsung tandai seluruh tahapan dan proyek selesai 100%">
                            <i class="fas fa-check-double"></i> Tandai Selesai (100%)
                        </button>
                        <button class="btn btn-sm" id="btn-modal-copy-summary" style="background: #64748b;" onclick="copyProgressSummary()">
                            <i class="fas fa-copy"></i> Salin Ringkasan Progres
                        </button>
                        <button class="btn btn-sm" id="btn-modal-reopen" style="background: #e67e22;" onclick="reopenOrResetProject()">
                            <i class="fas fa-undo"></i> Buka Ulang / Reset Progres
                        </button>
                        <button class="btn btn-sm" id="btn-modal-delete" style="background: #dc2626;" onclick="deleteProject(currentActiveProjectId)">
                            <i class="fas fa-trash-alt"></i> Hapus Proyek
                        </button>
                    </div>
                    <div style="display: flex; gap: 8px;">
                        <button class="btn" id="btn-modal-advance-stage" style="background: #16a34a;" onclick="advanceStage()">
    ...
    <script>
        ...
        function saveEmployeesData(immediate = false) {
            ...
            if (typeof currentActiveProjectId !== 'undefined' && currentActiveProjectId) {
                try {
                    const curProj = projectDataSMTI.find(p => p.id === currentActiveProjectId);
                    if (curProj) {
                        if (typeof renderProjectTeam === 'function') renderProjectTeam(curProj);
                        if (typeof populateChatAuthorSelect === 'function') populateChatAuthorSelect(curProj);
                    }
                } catch (err) {
                    console.warn("Update team avatars/comments warning:", err);
                }
            }
        ...
        function initAnalyticsChart() {
            ...
            // 2. CHART 1: DONUT STATUS PORTOFOLIO (Infographic Radial Style)
            ...
            if (canvasStatus) {
                ...
                const centerSub = document.getElementById('chartReportStatusCenterSub');
                if (centerSub) centerSub.innerText = `${doneCount} / ${totalCount} Selesai`;

                const ctxStatus = canvasStatus.getContext('2d');
                // Canvas Linear Gradients inspired by flat-infographic-element-set
                const gradTuntas = ctxStatus.createLinearGradient(0, 0, 0, 240);
                gradTuntas.addColorStop(0, '#10b981');
                gradTuntas.addColorStop(1, '#06b6d4');

                const gradBerjalan = ctxStatus.createLinearGradient(0, 0, 0, 240);
                gradBerjalan.addColorStop(0, '#ec4899');
                gradBerjalan.addColorStop(1, '#8b5cf6');

                const gradAwal = ctxStatus.createLinearGradient(0, 0, 0, 240);
                gradAwal.addColorStop(0, '#f59e0b');
                gradAwal.addColorStop(1, '#fbbf24');

                chartReportStatusInstance = new Chart(ctxStatus, {
                    type: 'doughnut',
                    data: {
                        labels: [
                        ...
                        datasets: [{
                            ...
                            backgroundColor: [gradTuntas, gradBerjalan, gradAwal],
                            borderWidth: 0,
                            borderRadius: 10,
                            spacing: 6,
                            hoverOffset: 6
            ...
            // 3. CHART 2: HORIZONTAL BAR CAPAIAN PER BAGIAN SMTI (Pill-ended Bars)
            ...
            if (canvasBagian) {
                ...
                const ctxBagian = canvasBagian.getContext('2d');
                const gradMiku = ctxBagian.createLinearGradient(0, 0, 320, 0);
                gradMiku.addColorStop(0, '#06b6d4');
                gradMiku.addColorStop(1, '#3b82f6');

                const gradPmsmt = ctxBagian.createLinearGradient(0, 0, 320, 0);
                gradPmsmt.addColorStop(0, '#10b981');
                gradPmsmt.addColorStop(1, '#059669');

                const gradSisdur = ctxBagian.createLinearGradient(0, 0, 320, 0);
                gradSisdur.addColorStop(0, '#ec4899');
                gradSisdur.addColorStop(1, '#8b5cf6');

                chartReportBagianInstance = new Chart(ctxBagian, {
                    type: 'bar',
                    data: {
                        labels: [
                        ...
                        datasets: [{
                            ...
                            backgroundColor: [gradMiku, gradPmsmt, gradSisdur],
                            borderRadius: 20,
                            barThickness: 24,
                            borderSkipped: false
                        }]
                    ...
                    options: {
                        ...
                        scales: {
                            ...
                            y: {
                                grid: { display: false },
                                ticks: { font: { size: 11.5, weight: '700' } }
                            }
                        },
                        plugins: {
                            legend: { display: false },
            ...
            // 4. CHART 3: DISTRIBUSI URGENSI PROYEK (Gradient Pillar Bars)
            ...
            if (canvasUrgensi) {
                ...
                const ctxUrgensi = canvasUrgensi.getContext('2d');
                const gradTinggi = ctxUrgensi.createLinearGradient(0, 0, 0, 240);
                gradTinggi.addColorStop(0, '#f43f5e');
                gradTinggi.addColorStop(1, '#fb7185');

                const gradSedang = ctxUrgensi.createLinearGradient(0, 0, 0, 240);
                gradSedang.addColorStop(0, '#f59e0b');
                gradSedang.addColorStop(1, '#fbbf24');

                const gradRendah = ctxUrgensi.createLinearGradient(0, 0, 0, 240);
                gradRendah.addColorStop(0, '#10b981');
                gradRendah.addColorStop(1, '#34d399');

                chartReportUrgensiInstance = new Chart(ctxUrgensi, {
                    type: 'bar',
                    data: {
                        ...
                        datasets: [{
                            ...
                            backgroundColor: [gradTinggi, gradSedang, gradRendah],
                            borderRadius: 14,
                            barThickness: 42,
                            borderSkipped: false
                        }]
        ...
        function renderNotifications() {
            ...
            // Evaluasi setiap proyek aktif
            activeProjects.forEach(p => {
                const picLower = (p.pic || '').toLowerCase();
                const isUserPic = currentUserName && (picLower.includes(currentUserName) || currentUserName.includes(picLower.split(' ')[0]));
                const isUserTeam = currentUserName && (p.teamMembers || []).some(m => m.toLowerCase().includes(currentUserName));
                const userTag = isUserPic ? '👑 PIC Anda' : (isUserTeam ? '👥 Tim Anda' : '');

                const stageIndex = p.flow.findIndex(s => s.status === 'active');
                const activeStage = stageIndex >= 0 ? p.flow[stageIndex].title : 'Tahap Berjalan';

                // Format pesan ringkas & jelas
                let alertMsg = '';
                if (p.alertNote) {
                    alertMsg = p.alertNote;
                } else {
            ...
            // Render isi menu dropdown notifikasi
            let html = `
                <div style="display: flex; justify-content: space-between; align-items: center; padding: 4px 2px 10px 2px; border-bottom: 1px solid var(--border-color); margin-bottom: 6px;">
                    <div>
                        <div style="font-weight: 800; font-size: 13.5px; color: var(--text-color); display: flex; align-items: center; gap: 6px;">
            ...
            if (totalNotifs === 0) {
                html += `
                    <div style="padding: 24px 12px; text-align: center; color: #64748b; font-size: 12px;">
                        <i class="fas fa-check-circle" style="font-size: 26px; color: #16a34a; margin-bottom: 8px; display: block;"></i>
                        Semua proyek tuntas 100%. Tidak ada pengingat mendesak saat ini.
                    </div>
                `;
            } else {
                html += `<div style="max-height: 360px; overflow-y: auto; display: flex; flex-direction: column; gap: 5px; padding-right: 2px;">`;
                    ...
                    html += `
                        <div class="dropdown-item" onclick="handleNotifClick(${item.id}, '${item.notifKey}')" style="align-items: flex-start; padding: 9px 10px; border-radius: 8px; transition: all 0.15s; border-bottom: 1px solid var(--border-color); gap: 10px; ${itemStyle}" title="Klik untuk buka alur & tandai dibaca">
                            ...
                            <div style="flex: 1; min-width: 0;">
                                ...
                                <div style="font-size: 11px; color: #64748b; margin-top: 3px; line-height: 1.35; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;">
                                    ${item.message}
                                </div>
                ...
                banner.innerHTML = `
                    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px;">
                        <div style="display: flex; align-items: center; gap: 12px;">
                            <i class="fas fa-check-circle" style="font-size: 30px; color: #16a34a;"></i>
                            <div>
                                <h4 style="margin: 0; color: #16a34a; font-size: 15px;">Semua Proyek Berjalan Terkendali</h4>
                                <p style="margin: 2px 0 0 0; font-size: 12.5px; color: #64748b;">Tidak ada proyek darurat yang memerlukan tindakan mendesak.</p>
                            </div>
                        </div>
                        <button class="btn btn-sm" style="background: #0284c7;" onclick="showPage('proyek', document.querySelectorAll('.menu-item')[1])">
                            Lihat Daftar Proyek
        ...
        function renderDashboardRunningProjects() {
            ...
            container.innerHTML = runningProjects.map(p => {
                const urgencyClass = p.urgency === 'Tinggi' ? 'high' : (p.urgency === 'Sedang' ? 'med' : 'low');
                const urgencyColor = p.urgency === 'Tinggi' ? '#dc2626' : (p.urgency === 'Sedang' ? '#d97706' : '#16a34a');
                const stageIndex = p.flow.findIndex(s => s.status === 'active');
                const activeStageName = stageIndex >= 0 ? p.flow[stageIndex].title : 'Tahap Berjalan';

                return `
                    <div style="background: var(--hover-color); border: 1px solid var(--border-color); border-left: 4px solid ${urgencyColor}; border-radius: 8px; padding: 12px 14px; display: flex; flex-direction: column; justify-content: space-between; gap: 10px;">
                        <div>
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
                                <span class="project-category-tag" style="font-size: 10.5px;">${p.category}</span>
                                <span class="urgency-badge ${urgencyClass}" style="font-size: 10px; padding: 2px 6px;">${p.urgency}</span>
                            </div>
                            <h4 style="font-size: 13.5px; font-weight: 700; color: var(--text-color); margin: 0; line-height: 1.35;">${p.name}</h4>
                            <div style="font-size: 11.5px; color: #64748b; margin-top: 5px;">
                                <i class="fas fa-sitemap" style="color: #0284c7;"></i> ${activeStageName}
                            </div>
                        </div>
                        <div>
                            <div style="display: flex; justify-content: space-between; font-size: 11px; margin-bottom: 4px;">
                                <span style="color: #64748b;">PIC: <strong>${p.pic.split('/')[0].trim()}</strong></span>
                                <strong style="color: #0284c7;">${p.progress}%</strong>
                            </div>
                            <div style="height: 6px; background: rgba(0,0,0,0.08); border-radius: 4px; overflow: hidden;">
                                <div style="width: ${p.progress}%; height: 100%; background: ${p.progress === 100 ? '#16a34a' : '#0284c7'}; border-radius: 4px;"></div>
                            </div>
        ...
        function renderProjectsList() {
            ...
            if (document.getElementById('proyek-kpi-high')) document.getElementById('proyek-kpi-high').innerText = allHighCount;
            if (document.getElementById('proyek-kpi-med')) document.getElementById('proyek-kpi-med').innerText = allMedCount;
            if (document.getElementById('proyek-kpi-low')) document.getElementById('proyek-kpi-low').innerText = allLowCount;
            ...
            if (document.getElementById('count-proj-high')) document.getElementById('count-proj-high').innerText = allHighCount;
            if (document.getElementById('count-proj-med')) document.getElementById('count-proj-med').innerText = allMedCount;
            if (document.getElementById('count-proj-low')) document.getElementById('count-proj-low').innerText = allLowCount;

            // Main Dashboard overview counts (Synchronized dengan proyek aktif)
            if (document.getElementById('dash-total-proyek')) document.getElementById('dash-total-proyek').innerText = totalCount;
            if (document.getElementById('dash-running-badge')) document.getElementById('dash-running-badge').innerText = runningCount + " Berjalan";
            if (document.getElementById('dash-proyek-subtext')) document.getElementById('dash-proyek-subtext').innerText = `${runningCount} Sedang Berjalan • ${completedCount} Selesai`;
            if (document.getElementById('dash-urgent-proyek')) document.getElementById('dash-urgent-proyek').innerText = runningHighCount;
            if (document.getElementById('dash-med-proyek')) document.getElementById('dash-med-proyek').innerText = runningMedCount;
            if (document.getElementById('side-urgent-count')) document.getElementById('side-urgent-count').innerText = runningHighCount + " Urgent";

            // Render banner darurat & daftar ringkas live di Dasbor
            renderDashboardUrgentBanner();
            renderDashboardRunningProjects();
            updateMainDashboardChart();
            renderNotifications();
        ...
        function renderMonitorBoard() {
            ...
            const completedList = document.getElementById('monitor-list-completed');
            ...
            const colCompleted = document.getElementById('mon-col-completed');
            ...
            // PISAHKAN SECARA TEGAS: Proyek Berjalan (Aktif) vs Proyek Selesai
            const activeProjects = filteredProjects.filter(p => !isProjectCompleted(p));
            const completedProjects = filteredProjects.filter(p => isProjectCompleted(p));

            // Kolom urgensi pada Live Board HANYA diisi oleh proyek yang masih berjalan
            const highProjects = activeProjects.filter(p => p.urgency === 'Tinggi');
            const medProjects = activeProjects.filter(p => p.urgency === 'Sedang');
            const lowProjects = activeProjects.filter(p => p.urgency === 'Rendah');
            ...
            if (document.getElementById('mon-count-completed')) {
                document.getElementById('mon-count-completed').innerText = completedProjects.length + " Proyek Tuntas";
            }
            ...
            const pCompleted = document.getElementById('mon-pill-count-completed');

            if (pAll) pAll.innerText = activeProjects.length; // Hanya menghitung proyek aktif berjalan
            ...
            if (pCompleted) pCompleted.innerText = completedProjects.length;
            ...
            if (currentMonitorUrgencyFilter === 'Tinggi') {
                ...
                if (colCompleted) colCompleted.style.display = 'none';
            ...
            } else if (currentMonitorUrgencyFilter === 'Selesai') {
                colHigh.style.display = 'none';
                colMed.style.display = 'none';
                colLow.style.display = 'none';
                if (colCompleted) {
                    colCompleted.style.display = 'flex';
                    board.style.gridTemplateColumns = '1fr';
                    if (completedList) completedList.className = 'monitor-card-list monitor-card-list-expanded';
                }
            } else {
                // Default: Semua Kolom (HANYA 3 KOLOM PROYEK AKTIF BERJALAN)
                colHigh.style.display = 'flex';
                colMed.style.display = 'flex';
                colLow.style.display = 'flex';
                if (colCompleted) colCompleted.style.display = 'none';
            ...
            const createCompletedMonitorCard = (p) => {
                return `
                    <div class="monitor-project-card monitor-card-completed" onclick="openFlowModal(${p.id})">
                        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 6px;">
                            <span style="font-size: 10px; font-weight: 700; color: #34d399; text-transform: uppercase;">${p.category}</span>
                            <span style="font-size: 11px; font-weight: 800; color: #34d399; background: rgba(16, 185, 129, 0.2); padding: 2px 8px; border-radius: 4px; border: 1px solid rgba(16, 185, 129, 0.4);">
                                <i class="fas fa-check-circle"></i> Selesai 100%
                            </span>
                        </div>
                        <div class="monitor-card-title">${p.name}</div>
                        <div class="monitor-card-stage" style="color: #6ee7b7;">
                            <i class="fas fa-flag-checkered" style="color: #34d399;"></i> Status: <span class="monitor-stage-highlight" style="background: rgba(16, 185, 129, 0.15); color: #34d399; border-color: rgba(16, 185, 129, 0.3);">Seluruh Alur Tuntas Diselesaikan</span>
                        </div>

                        <div class="monitor-progress-box">
                            <div class="monitor-progress-label">
                                <span style="color: #94a3b8;">Progres Kerja</span>
                                <strong style="color: #34d399;">100%</strong>
                            </div>
                            <div class="monitor-progress-track">
                                <div class="monitor-progress-bar bar-completed" style="width: 100%;"></div>
                            </div>
                        </div>

                        <div class="monitor-card-footer">
                            <span><i class="fas fa-user-circle"></i> ${p.pic}</span>
                            <span class="monitor-click-hint" style="color: #34d399;"><i class="fas fa-eye"></i> Buka Arsip Flow</span>
                        </div>
                    </div>
                `;
            };

            const emptyStateHtml = (title, msg) => `
                <div style="text-align: center; padding: 45px 20px; color: #94a3b8; font-size: 13px; grid-column: 1 / -1; width: 100%; border: 1px dashed rgba(255,255,255,0.12); border-radius: 12px; margin: 10px 0;">
                    <i class="fas fa-check-circle" style="font-size: 32px; color: #4ade80; margin-bottom: 10px; display: block;"></i>
                    ${msg || `Tidak ada proyek dengan status ${title} saat ini.`}
                </div>
            `;

            highList.innerHTML = highProjects.length > 0
                ? highProjects.map(p => createMonitorCard(p, 'monitor-card-high', 'bar-high')).join('')
                : emptyStateHtml('Urgensi Tinggi', 'Semua proyek prioritas tinggi tuntas diselesaikan.');
            ...
            if (completedList) {
                completedList.innerHTML = completedProjects.length > 0
                    ? completedProjects.map(p => createCompletedMonitorCard(p)).join('')
                    : emptyStateHtml('Selesai', 'Belum ada proyek yang berstatus selesai 100%.');
            }

            // Update Ticker Berjalan (Hanya mengingatkan proyek AKTIF yang sedang berjalan!)
            const tickerEl = document.getElementById('monitor-ticker-content');
            if (tickerEl) {
                const urgentRunning = activeProjects.filter(p => p.urgency === 'Tinggi' || p.urgency === 'Sedang');
                if (urgentRunning.length > 0) {
                    tickerEl.innerHTML = urgentRunning.map(p => {
                        const icon = p.urgency === 'Tinggi' ? '⚠️ PENGINGAT TINGGI:' : '📌 PENGAWALAN:';
                        return `${icon} <strong>${p.name}</strong> (${p.category}) - Progres: ${p.progress}% [PIC: ${p.pic}] • Target: ${p.deadline}`;
                    }).join(' ••• ');
                } else {
                    tickerEl.innerHTML = `✅ Seluruh proyek prioritas tinggi sedang berjalan lancar & terkendali • Departemen SMTI PT Pupuk Kujang`;
                }
            }
        }

        // Keyboard navigation for Monitor Mode (1: Semua Berjalan, 2: Tinggi, 3: Sedang, 4: Rutin, 5: Selesai, Esc: Tutup)
        document.addEventListener('keydown', function(e) {
            ...
            if (e.key === '1') {
                ...
                setMonitorUrgencyFilter('Selesai');
        ...
        /* =========================================================
           HAK AKSES PROGRES PROYEK KHUSUS PIC & TIM TER-INVITE (RBAC)
           ========================================================= */
        function isSameOrMatchingPerson(nameA, nameB) {
            if (!nameA || !nameB) return false;
            const cleanA = String(nameA).replace(/\(.*?\)/g, '').trim().toLowerCase();
            const cleanB = String(nameB).replace(/\(.*?\)/g, '').trim().toLowerCase();
            if (cleanA === cleanB) return true;
            if (cleanA.length >= 4 && cleanB.length >= 4) {
                if (cleanA.includes(cleanB) || cleanB.includes(cleanA)) return true;
            }
            const tokA = cleanA.split(/\s+/).filter(Boolean);
            const tokB = cleanB.split(/\s+/).filter(Boolean);
            if (tokA.length > 1 && tokB.length > 1) {
                if (tokA[0] === tokB[0] && tokA[1] === tokB[1]) return true;
            }
            return false;
        }
        ...
        function canUserEditProject(project) {
            ...
            if (typeof currentAuthUser === 'undefined' || !currentAuthUser || !currentAuthUser.name) return false;

            // Super Admin (Septian) & Manager (Henisya Permata Sari) memiliki wewenang tata kelola menyeluruh
            if (currentAuthUser.isSuperAdmin || (currentAuthUser.name && currentAuthUser.name.toLowerCase().includes('septian'))) return true;
            if (currentAuthUser.isManager || (currentAuthUser.name && currentAuthUser.name.toLowerCase().includes('henisya'))) return true;

            const currentName = currentAuthUser.name.trim().toLowerCase();

            // Cek apakah user adalah PIC proyek (mendukung nama tunggal atau dipisah garis miring / koma / 'dan')
            const picRaw = (project.pic || '').trim().toLowerCase();
            if (picRaw) {
                const picParts = picRaw.split(/[/,&]|(?:\bdan\b)/i).map(p => p.replace(/\(.*?\)/g, '').trim()).filter(Boolean);
                for (const p of picParts) {
                    if (isSameOrMatchingPerson(currentName, p)) return true;
                }
            }

            // Cek apakah user terdaftar dalam anggota tim proyek yang diundang
            if (Array.isArray(project.teamMembers)) {
                for (const member of project.teamMembers) {
                    if (!member) continue;
                    const m = String(member).replace(/\(.*?\)/g, '').trim().toLowerCase();
                    if (isSameOrMatchingPerson(currentName, m)) return true;
                }
            }
        ...
        function showPermissionDeniedAlert(project, actionDesc = "mengubah progres atau checklist proyek ini") {
            const picName = project ? (project.pic || 'PIC') : 'PIC';
            const currentUser = (typeof currentAuthUser !== 'undefined' && currentAuthUser && currentAuthUser.name) 
                ? currentAuthUser.name 
                : 'Pengguna';
            alert(`🔒 Akses Terkunci (Mode Hanya-Lihat)\n\nAkun Anda: ${currentUser}\nHak Akses: Anda bukan PIC atau anggota tim yang diundang pada proyek ini.\n\nHanya PIC (${picName}) atau anggota tim ter-invite yang dapat ${actionDesc}.`);
        }

        function updateFlowModalPermissions(project, canEdit) {
            const currentUserName = (typeof currentAuthUser !== 'undefined' && currentAuthUser && currentAuthUser.name) 
                ? currentAuthUser.name 
                : 'Tamu';

            // 1. Banner Notifikasi Hak Akses
            const noticeEl = document.getElementById('modal-permission-notice');
            if (noticeEl) {
                if (!canEdit) {
                    ...
                    noticeEl.style.background = 'rgba(245, 158, 11, 0.09)';
                    noticeEl.style.border = '1px solid rgba(245, 158, 11, 0.3)';
                    noticeEl.style.padding = '9px 14px';
                    noticeEl.style.borderRadius = '8px';
                    noticeEl.style.marginBottom = '12px';
                    noticeEl.innerHTML = `
                        <div style="display: flex; align-items: center; gap: 10px; font-size: 12px; color: #b45309; line-height: 1.4;">
                            <i class="fas fa-lock" style="font-size: 15px; color: #d97706; flex-shrink: 0;"></i>
                            <span><strong>Mode Hanya-Lihat (Read-Only):</strong> Anda login sebagai <strong>${currentUserName}</strong>. Menu perubahan progres proyek ini disembunyikan & dikunci khusus untuk PIC (<strong>${project.pic}</strong>) serta anggota tim ter-invite.</span>
                        </div>
                        <span style="font-size: 10.5px; font-weight: 700; background: #fef3c7; color: #b45309; border: 1px solid #fde68a; padding: 2px 7px; border-radius: 4px; white-space: nowrap;">
            ...
            // 2. Toolbar Atur Status Tahap & Manual Progress (Sembunyikan bila bukan PIC/tim)
            const toolbar = document.getElementById('modal-stage-toolbar');
            if (toolbar) {
                toolbar.style.display = canEdit ? 'flex' : 'none';
            }

            // 3. Tombol Ganti PIC & Undang Karyawan (Sembunyikan bila bukan PIC/tim)
            const btnChangePic = document.getElementById('btn-modal-change-pic');
            if (btnChangePic) {
                btnChangePic.style.display = canEdit ? 'inline-block' : 'none';
            }
            const btnInviteTeam = document.getElementById('btn-modal-invite-team');
            if (btnInviteTeam) {
                btnInviteTeam.style.display = canEdit ? 'inline-flex' : 'none';
            }

            // 4. Tombol Ubah Catatan Alert
            const btnEditAlert = document.getElementById('btn-modal-edit-alert');
            if (btnEditAlert) {
                btnEditAlert.style.display = canEdit ? 'inline-flex' : 'none';
            }

            // 5. Form Tambah Tugas Baru
            const formAddTask = document.getElementById('modal-add-task-form');
            if (formAddTask) {
                formAddTask.style.display = canEdit ? 'flex' : 'none';
            }
            const lockedTaskNotice = document.getElementById('modal-task-locked-notice');
            if (lockedTaskNotice) {
                lockedTaskNotice.style.display = canEdit ? 'none' : 'block';
            }

            // 6. Tombol Aksi di Bawah (Yang di-screenshot user):
            // Sembunyikan: Tandai Selesai, Buka Ulang, Hapus Proyek, Tahap Berikutnya
            const btnMarkCompleted = document.getElementById('btn-modal-mark-completed');
            if (btnMarkCompleted) {
                btnMarkCompleted.style.display = canEdit ? 'inline-flex' : 'none';
            }
            const btnReopen = document.getElementById('btn-modal-reopen');
            if (btnReopen) {
                btnReopen.style.display = canEdit ? 'inline-flex' : 'none';
            }
            const btnDelete = document.getElementById('btn-modal-delete');
            if (btnDelete) {
                btnDelete.style.display = canEdit ? 'inline-flex' : 'none';
            }
            const btnAdvance = document.getElementById('btn-modal-advance-stage');
            if (btnAdvance) {
...
END:VCALENDAR`;
        ...
        function openFlowModal(projectId) {
            ...
            updateFlowModalPermissions(project, canEdit);
        ...
        function renderStageTasks(project, stepIndex) {
            ...
            if (!step.tasks || step.tasks.length === 0) {
                checklistContainer.innerHTML = `
                    <div style="padding: 12px; text-align: center; color: #94a3b8; font-size: 12.5px; border: 1px dashed var(--border-color); border-radius: 6px; margin-bottom: 8px;">
                        Belum ada tugas/dokumen syarat di tahap ini.${canEdit ? ' Tambahkan di bawah.' : ''}
                    </div>
                `;
                return;
            }
            ...
            checklistContainer.innerHTML = step.tasks.map((task, tIdx) => {
                const isChecked = !!task[1];
                if (!canEdit) {
                    return `
                        <div class="task-item ${isChecked ? 'checked' : ''}" style="cursor: default;" title="Mode Hanya-Lihat: Checklist dikunci untuk non-tim proyek">
                            <div class="task-left">
                                <input type="checkbox" ${isChecked ? 'checked' : ''} disabled style="cursor: not-allowed; opacity: 0.65;">
                                <span>${task[0]}</span>
                            </div>
                        </div>
                    `;
                ...
                return `
                    <div class="task-item ${isChecked ? 'checked' : ''}" onclick="toggleTask(${stepIndex}, ${tIdx})">
                        <div class="task-left">
                            <input type="checkbox" ${isChecked ? 'checked' : ''} onclick="event.stopPropagation(); toggleTask(${stepIndex}, ${tIdx})">
                            <span>${task[0]}</span>
                        </div>
        ...
        function toggleTask(stepIndex, taskIndex) {
            ...
            if (!canUserEditProject(project)) {
                showPermissionDeniedAlert(project, "mencentang kegiatan proyek");
                return;
            }
        ...
        function addNewTask() {
            ...
            if (!canUserEditProject(project)) {
                showPermissionDeniedAlert(project, "menambah kegiatan baru");
                return;
            }
        ...
        function deleteTask(stepIndex, taskIndex) {
            ...
            if (!canUserEditProject(project)) {
                showPermissionDeniedAlert(project, "menghapus kegiatan proyek");
                return;
            }
        ...
        function setStepStatusManually(newStatus) {
            ...
            if (!canUserEditProject(project)) {
                showPermissionDeniedAlert(project, "mengubah status tahapan");
                return;
            }
        ...
        function applyManualProgress() {
            ...
            if (!canUserEditProject(project)) {
                showPermissionDeniedAlert(project, "mengatur total progres");
                return;
            }
        ...
        function advanceStage() {
            ...
            if (!canUserEditProject(project)) {
                showPermissionDeniedAlert(project, "memajukan tahapan proyek");
                return;
            }
        ...
        function markProjectCompletedQuick() {
            ...
            if (!canUserEditProject(project)) {
                showPermissionDeniedAlert(project, "menyelesaikan proyek");
                return;
            }
        ...
        function reopenOrResetProject() {
            ...
            if (!canUserEditProject(project)) {
                showPermissionDeniedAlert(project, "membuka ulang atau mereset proyek");
                return;
            }
        ...
        function editAlertNote() {
            ...
            if (!canUserEditProject(project)) {
                showPermissionDeniedAlert(project, "mengubah catatan urgensi");
                return;
            }
        ...
        function openInviteTeamModal() {
            ...
            if (!canUserEditProject(project)) {
                showPermissionDeniedAlert(project, "mengundang anggota tim proyek");
                return;
            }
        ...
        function toggleInviteEmployee(empName) {
            ...
            if (!canUserEditProject(project)) {
                showPermissionDeniedAlert(project, "mengubah tim proyek");
                return;
            }
        ...
        function removeMemberFromProject(empName) {
            ...
            if (!canUserEditProject(project)) {
                showPermissionDeniedAlert(project, "mengeluarkan anggota tim proyek");
                return;
            }
        ...
        function openChangePicModal() {
            ...
            if (!canUserEditProject(project)) {
                showPermissionDeniedAlert(project, "mengganti PIC proyek ini");
                return;
            }
        ...
        async function deleteProject(projectId) {
            ...
            if (!canUserEditProject(project)) {
                showPermissionDeniedAlert(project, "menghapus proyek ini");
                return;
            }
        ...
        function updateTotalCount() {
            ...
            if (total > 0) {
                ...
                const subEl = document.getElementById('dash-karyawan-subtext');
                if (subEl) {
                    subEl.innerText = `${tkoCount} TKO • ${tknoCount} TKNO`;
                }
            }
        }
        ...
        function applyAuthenticatedUser(user) {
            ...
            if (document.getElementById('flowModal') && document.getElementById('flowModal').classList.contains('active') && typeof currentActiveProjectId !== 'undefined' && currentActiveProjectId) {
                openFlowModal(currentActiveProjectId);
            }
        }
