        ...
        function renderCustomStepsRows() {
            ...
            container.innerHTML = customStepsState.map((step, idx) => {
                ...
                const tasksHtml = normalizedTasks.map((task, tIdx) => `
                    <div style="display: flex; align-items: center; gap: 8px; background: var(--hover-color); padding: 5px 8px; border-radius: 5px; border: 1px solid var(--border-color);">
                        <input type="checkbox" ${task.selected ? 'checked' : ''} onchange="toggleCustomTaskSelection(${idx}, ${tIdx}, this.checked)" title="Pilih kegiatan ini untuk disertakan dalam alur proyek" style="cursor: pointer;">
                        <input type="text" value="${(task.text || '').replace(/"/g, '&quot;')}" oninput="customStepsState[${idx}].tasks[${tIdx}].text = this.value" placeholder="Nama kegiatan / dokumen syarat..." style="flex: 1; border: none; background: transparent; font-size: 12px; color: var(--text-color); outline: none;">
                        <button type="button" onclick="removeCustomTaskFromStep(${idx}, ${tIdx})" style="background: transparent; border: none; color: #94a3b8; cursor: pointer; padding: 3px; font-size: 12px;" title="Hapus kegiatan ini">
                            <i class="fas fa-times"></i>
                        </button>
                    </div>
                `).join('');
        ...
        function saveNewProject(e) {
            ...
            const flow = customStepsState.map((st, i) => {
                const stageTasks = (st.tasks || [])
                    .filter(t => t.selected !== false && t.text && t.text.trim())
                    .map(t => [t.text.trim(), false]);
