...
<body>
    ...
    <script>
        ...
        /* =========================================================
           FITUR TAMBAH PROYEK (ALUR KERJA MANUAL & TEMPLATE REUSABLE)
           ========================================================= */
        ...
        function populateWorkflowTemplateDropdown(selectedVal = '__manual__') {
            const select = document.getElementById('new-proj-template');
            if (!select) return;

            let html = `<option value="__manual__">➕ Alur Kerja Manual Baru (Tentukan Bebas Sesuai Lapangan)</option>`;

            if (userSavedWorkflowTemplates && userSavedWorkflowTemplates.length > 0) {
                html += `<optgroup label="⭐ Template Alur Tersimpan (${userSavedWorkflowTemplates.length} Template)">`;
        ...
        function updateTemplateUiState(selectedVal) {
            ...
            if (selectedVal !== '__manual__') {
                ...
                if (tpl) {
                    ...
                    if (hint) hint.innerHTML = `<span style="color: #10b981; font-weight: 600;"><i class="fas fa-check-circle"></i> Memuat template: "${tpl.name}".</span> Anda tetap bisa memodifikasi tahapan khusus untuk proyek ini.`;
                    return;
                }
            }

            if (btnDel) btnDel.style.display = 'none';
            if (hint) hint.innerText = "Alur kerja bebas disusun manual sesuai fakta lapangan. Anda bisa menyimpannya sebagai template baru kapan saja.";
        }

        function onTemplateSelectChange(val) {
            if (val === '__manual__') {
                // If switching to manual and steps are empty, initialize with 3 clean editable steps
                if (!customStepsState || customStepsState.length === 0) {
                    customStepsState = [
                        { title: "Tahap 1: Inisiasi & Identifikasi Lapangan", date: "Bulan 1", desc: "Pelaksanaan tahapan awal." },
                        { title: "Tahap 2: Eksekusi & Penerapan", date: "Bulan 2", desc: "Pelaksanaan tugas utama." },
                        { title: "Tahap 3: Evaluasi & Serah Terima", date: "Bulan 3", desc: "Penyelesaian dan pelaporan." }
                    ];
                }
                updateTemplateUiState('__manual__');
        ...
        function renderCustomStepsRows() {
            ...
            if (!customStepsState || customStepsState.length === 0) {
                container.innerHTML = `
                    <div style="text-align: center; padding: 20px; color: #94a3b8; font-size: 12px; border: 1px dashed var(--border-color); border-radius: 6px;">
                        Belum ada tahapan alur kerja. Klik tombol <strong>+ Tambah Tahap</strong> di atas untuk membuat tahapan sesuai fakta lapangan.
                    </div>
                `;
                return;
            }
            ...
            container.innerHTML = customStepsState.map((step, idx) => `
                <div style="display: flex; gap: 8px; align-items: center; background: var(--bg-color); padding: 8px 10px; border-radius: 6px; border: 1px solid var(--border-color);">
                    ...
                    <span style="font-size: 11px; font-weight: 700; width: 50px; color: var(--secondary-color); white-space: nowrap;">Tahap ${idx + 1}</span>
        ...
        function addCustomStepRow() {
            customStepsState.push({
                title: `Tahap ${customStepsState.length + 1}: Kegiatan Lapangan`,
        ...
        function promptSaveCurrentStepsAsTemplate() {
            ...
            const templateName = prompt("Masukkan Nama Template Baru untuk Alur Kerja Ini:\n(Contoh: Alur Kalibrasi Tangki, Template Audit Lapangan SMTI)");
            ...
            alert(`Sukses! Template "${trimmedName}" berhasil disimpan dengan ${newTemplate.steps.length} tahapan.\nTemplate ini sekarang bisa dipilih langsung untuk proyek lainnya!`);
        }
        ...
        function deleteCurrentCustomTemplate() {
            ...
            if (confirm(`Apakah Anda yakin ingin menghapus template "${tpl.name}" dari daftar template tersimpan?`)) {
                ...
                saveUserSavedWorkflowTemplates();
                populateWorkflowTemplateDropdown('__manual__');
        ...
        function openAddProjectModal() {
            ...
            // Inisialisasi tahapan manual yang bersih & fleksibel
            customStepsState = [
                { title: "Tahap 1: Inisiasi & Identifikasi Lapangan", date: "Bulan 1", desc: "Pelaksanaan tahap awal." },
                { title: "Tahap 2: Eksekusi & Penerapan", date: "Bulan 2", desc: "Pelaksanaan tugas utama." },
                { title: "Tahap 3: Evaluasi & Serah Terima", date: "Bulan 3", desc: "Penyelesaian dan pelaporan." }
            ];
