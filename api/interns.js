export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    try {
      ...
      if (typeof internsData === "string") {
        internsData = JSON.parse(internsData);
      }
      if (internsData && internsData.interns) {
        internsData = internsData.interns;
      }

      if (!Array.isArray(internsData)) {
        return res.status(400).json({ success: false, error: "Data harus berupa array anak magang" });
      }
      ...
      return res.status(200).json({ success: true, count: internsData.length });
    } catch (e) {
      return res.status(500).json({ success: false, error: e.message });
    }
  }
